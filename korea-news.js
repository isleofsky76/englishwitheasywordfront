...
window.VIEWPOST_SEO = {
    boardPath: 'korea-news',
    boardLabel: 'Korea News',
    fallbackHtml: 'korea-news.html',
    listPath: '/korea-news',
    bySlugPath: '/korea-news/by-slug',
};
...
function nvInjectArticleJsonLd(post, isoDate, description) {
    ...
    const pageUrl = index
        ? `${NV_SITE_ORIGIN}/korea-news.html?index=${index}`
        : `${NV_SITE_ORIGIN}/korea-news.html`;
    const itemList = nvParseRankingItemList(post.message);
    const payload = {
        '@context': 'https://schema.org',
        '@type': 'Article',
        headline: post.title || 'Korea News',
        description,
        ...
        inLanguage: ['ko', 'en'],
        keywords: 'Korea News, 한국뉴스, military English, defense vocabulary, Pentagon, NATO',
        about: {
            '@type': 'Thing',
            name: 'Korea News vocabulary',
        },
    };
...
function updatePageSeo(post, formattedDate) {
    ...
    const title = (post.title || 'Korea News').trim();
    const desc = nvPlainText(post.message, 155);
    const fullTitle = `${title} | 한국뉴스 · English Easy Study`;
...
function showError(message, details = '') {
    ...
    postContainer.innerHTML = `
        <div style="text-align: center; padding: 30px; color: #d32f2f; background-color: #ffebee; border-radius: 8px; margin: 20px;">
            ...
            <button class="btn btn-primary mt-3" onclick="window.location.href='korea-news-list.html${apiParam}'">목록으로 돌아가기</button>
        </div>
    `;
}
...
async function loadPost() {
    ...
    try {
        ...
        if (slug) {
            const slugUrl = `${API_BASE_URL}/korea-news/by-slug/${encodeURIComponent(slug)}`;
        ...
        if (!post) {
        const url = `${API_BASE_URL}/korea-news`;
        ...
        // 조회수 증가 API 호출
        try {
            const viewResponse = await fetch(`${API_BASE_URL}/korea-news/${post._id}/view`, {
                method: 'POST',
                headers: {
        ...
        if (backBtn) backBtn.onclick = () => {
            const apiParam = apiMode ? `?api=${apiMode}` : '';
            window.location.href = `korea-news-list.html${apiParam}`;
        };
        ...
        if (editBtn) editBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/korea-news-viewpost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (editForm) editForm.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/korea-news-updatepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (deleteBtn) deleteBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/korea-news-deletepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
