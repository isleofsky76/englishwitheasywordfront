...
console.log('📡 요리 어휘 API_BASE_URL:', API_BASE_URL);
console.log('page30_guestbook_cooking_voca.js v20260616');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (guestbookForm) guestbookForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/cooking-voca`, {
                method: 'POST',
                headers: {
    ...
    async function loadMessages(forceRefresh = false) {
        ...
        try {
            ...
            let errorMessage = `요청 URL: <code>${API_BASE_URL}/cooking-voca</code><br><br>`;
    ...
    window.adminDeletePost = async function (id) {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/cooking-voca-admin/deletepost`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
