...
window.VIEWPOST_SEO = {
    boardPath: 'english-opinions',
    boardLabel: 'English Opinions',
    fallbackHtml: 'english-opinions.html',
    listPath: '/opinions',
    bySlugPath: '/opinions/by-slug',
};
...
function convertMediaLinks(text) {
    ...
    // URL을 찾아서 링크로 변환 (이미 링크 태그·href 속성 안의 URL은 건너뛰기)
    result = result.replace(urlPattern, (url, _capture, offset) => {
        ...
        // <a> 본문 안이거나 href/src 등 속성 값 안이면 그대로 둠
...
// 에러 메시지 표시 함수
function showError(message, details = '', listPage = 'english-opinions-list.html') {
    const postContainer = document.getElementById('post-container');
...
async function loadPost() {
    ...
    try {
        if (!window.ViewpostSeo) {
            showError('게시글을 불러올 수 없습니다', 'viewpost-seo.js 가 필요합니다.', 'english-opinions-list.html');
            return;
        }
        ...
        if (result.error === 'missing-param') {
            showError('게시글을 찾을 수 없습니다', 'slug 또는 index가 없습니다.', 'english-opinions-list.html');
            return;
        }
        if (result.error === 'slug-not-found') {
            showError('게시글을 찾을 수 없습니다', `slug "${result.slug}" 글이 없습니다.`, 'english-opinions-list.html');
            return;
        }
        if (result.error) {
            showError('게시글을 불러올 수 없습니다', String(result.error), 'english-opinions-list.html');
            return;
        }
        ...
        // 조회수 증가 API 호출
        try {
            const viewResponse = await fetch(`${API_BASE_URL}/opinions/${post._id}/view`, {
                method: 'POST',
                headers: {
