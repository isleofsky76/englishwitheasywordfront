...
function nvInjectArticleJsonLd(post, isoDate, description) {
    ...
    const pageUrl = index
        ? `${NV_SITE_ORIGIN}/culture-voca.html?index=${index}`
        : `${NV_SITE_ORIGIN}/culture-voca.html`;
    const itemList = nvParseRankingItemList(post.message);
    const payload = {
        '@context': 'https://schema.org',
        '@type': 'Article',
        headline: post.title || 'Culture Vocabulary',
        description,
        ...
        inLanguage: ['ko', 'en'],
        keywords: 'culture vocabulary, 컬쳐 어휘, cultural English, tradition, ceremony, arts vocabulary',
        about: {
            '@type': 'Thing',
            name: 'Culture and tradition vocabulary',
        },
    };
...
function showError(message, details = '') {
    ...
    postContainer.innerHTML = `
        <div style="text-align: center; padding: 30px; color: #d32f2f; background-color: #ffebee; border-radius: 8px; margin: 20px;">
            ...
            <button class="btn btn-primary mt-3" onclick="window.location.href='culture-voca-list.html${apiParam}'">목록으로 돌아가기</button>
        </div>
    `;
}
...
async function loadPost() {
    ...
    try {
        ...
        // 조회수 증가 API 호출
        try {
            const viewResponse = await fetch(`${API_BASE_URL}/culture-voca/${post._id}/view`, {
                method: 'POST',
                headers: {
        ...
        if (backBtn) backBtn.onclick = () => {
            const apiParam = apiMode ? `?api=${apiMode}` : '';
            window.location.href = `culture-voca-list.html${apiParam}`;
        };
        ...
        if (editBtn) editBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/culture-voca-viewpost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (editForm) editForm.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/culture-voca-updatepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (deleteBtn) deleteBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/culture-voca-deletepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
