...
window.VIEWPOST_SEO = {
    boardPath: 'english-synonym',
    boardLabel: 'English Synonym',
    fallbackHtml: 'english-synonym.html',
    listPath: '/vocabulary',
    bySlugPath: '/vocabulary/by-slug',
};
...
/** 유의어 블로그형: "robust — 탄탄한" / "**well-built** — ..." 앞 영어만 추출 */
function vvExtractLeadingEnglishWord(text) {
    const t = String(text || '')
        .trim()
        .replace(/^[*_]+/, '')
        .replace(/[*_]+$/, '');
    const m = t.match(/^([A-Za-z][A-Za-z'-]*(?:\s+[A-Za-z][A-Za-z'-]*){0,2})\b/);
    if (!m) return '';
    const word = m[1].trim();
    if (word.length < 2) return '';
    if (/^(the|and|or|a|an|of|to|in|on|for|with|from)$/i.test(word)) return '';
    return word;
}

function vvEscapeRegExp(s) {
    return String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

function vvAppendTtsAfterLeadingWord(lineHtml, word) {
    ...
    const patterns = [
        ...
        new RegExp(`(<em[^>]*>\\s*\\*?${w}\\*?\\s*</em>)`, 'i'),
        new RegExp(`(\\b${w}\\b)`, 'i'),
    ];
    for (const re of patterns) {
        if (re.test(lineHtml)) return lineHtml.replace(re, `$1${btn}`);
    }
    return lineHtml + btn;
}
...
function attachVocabularyWebTTS(container) {
    ...
    paragraphs.forEach((p) => {
        ...
        const newLines = lines.map((lineHtml) => {
            ...
            if (vvIsMostlyEnglish(plain)) {
                return vvAppendTtsInlineAfterEnglish(lineHtml, plain);
            }
            const lead = vvExtractLeadingEnglishWord(plain);
            if (lead) return vvAppendTtsAfterLeadingWord(lineHtml, lead);
            return lineHtml;
        });
...
async function loadPost() {
    ...
    try {
        if (!window.ViewpostSeo) {
            showError('게시글을 불러올 수 없습니다', 'viewpost-seo.js 가 필요합니다.', 'english-synonym-list.html');
            return;
        }
        ...
        if (result.error === 'missing-param') {
            showError('게시글을 찾을 수 없습니다', 'slug 또는 index가 없습니다.', 'english-synonym-list.html');
            return;
        }
        if (result.error === 'slug-not-found') {
            showError('게시글을 찾을 수 없습니다', `slug "${result.slug}" 글이 없습니다.`, 'english-synonym-list.html');
            return;
        }
        if (result.error) {
            showError('게시글을 불러올 수 없습니다', String(result.error), 'english-synonym-list.html');
            return;
        }
