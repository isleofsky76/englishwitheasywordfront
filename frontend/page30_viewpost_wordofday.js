...
function attachWordOfDayWebTTS() {
    ...
    const nodes = Array.from(container.querySelectorAll('p, div, span')).reverse();
    nodes.forEach((node) => {
        ...
        // 자식에 이미 스피커가 있으면 부모에는 추가하지 않음 (이중 표시 방지)
        if (node.querySelector && node.querySelector('.wotd-tts-btn')) {
            node.setAttribute('data-wotd-tts', '1');
            return;
        }

        // 하이라이트용 짧은 span은 건너뛰고, 문장 단위에만 스피커 1개
        if (node.tagName === 'SPAN' && node.children.length === 0) {
            const leafPlain = wotdStripTagsToText(node.textContent || '');
            const words = leafPlain.replace(/\s*🔊\s*$/u, '').trim().split(/\s+/).filter(Boolean);
        ...
        const newLines = lines.map((lineHtml) => {
            ...
            // 본문에 넣은 🔊 이모지는 제거하고 버튼만 남김 (이중 표시 방지)
            let cleaned = String(lineHtml).replace(/\s*🔊\s*/gu, '');
            const plain = wotdStripTagsToText(cleaned);
            if (!plain) return cleaned;
            if (!wotdIsMostlyEnglish(plain)) return cleaned;
            if (/^Source\b/i.test(plain) || /^출처\b/.test(plain) || /^WSJ\s*\|/i.test(plain) || /^https?:\/\//i.test(plain)) return cleaned;
            const speak = plain
                .replace(/\p{Extended_Pictographic}/gu, '')
                .replace(/\s+/g, ' ')
                .trim();
            if (!speak) return cleaned;
            // 짧은 구문(단어 하이라이트)에는 스피커를 붙이지 않음
            if (speak.split(/\s+/).filter(Boolean).length < 6) return cleaned;
            changed = true;
            return wotdAppendTtsInlineAfterEnglish(cleaned, speak);
        });
