...
console.log('📡 뉴스 어휘 API_BASE_URL:', API_BASE_URL);
console.log('page30_guestbook_news_voca.js v20260616');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (editFormEl) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // 폼 리셋 및 컨테이너 숨기기
                form.reset();
                const editModalEl = document.getElementById('edit-post-modal');
    ...
    async function loadMessages(forceRefresh = false) {
        ...
        try {
            ...
            let errorMessage = `요청 URL: <code>${API_BASE_URL}/guestbook</code><br><br>`;
    ...
    window.editPost = async function (id) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                document.getElementById('write-post-container')?.style && (document.getElementById('write-post-container').style.display = 'none');
                const editModalEl = document.getElementById('edit-post-modal');
                if (editModalEl && typeof bootstrap !== 'undefined') {
    ...
    window.adminDeletePost = async function (id) {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/admin/deletepost`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
