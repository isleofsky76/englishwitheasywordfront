...
// Cooking Voca 시맨틱 HTML + 기본 서식 태그 허용
...
window.VIEWPOST_SEO = {
    boardPath: 'cooking-voca',
    boardLabel: 'Cooking Voca',
    fallbackHtml: 'cooking-voca.html',
    listPath: '/cooking-voca',
    bySlugPath: '/cooking-voca/by-slug',
};
...
function nvParseRankingItemList(html) {
    const div = document.createElement('div');
    div.innerHTML = html || '';
    const rankLines = div.querySelectorAll('.cv-rank-line');
    if (rankLines.length) {
        return {
            '@type': 'ItemList',
    ...
    const rows = div.querySelectorAll('.cv-rank-table tbody tr');
    if (!rows.length) return null;
    return {
        '@type': 'ItemList',
        numberOfItems: rows.length,
        itemListElement: Array.from(rows).map((tr, i) => {
            const rank = tr.querySelector('.cv-col-rank')?.textContent?.trim() || String(i + 1);
            const cells = tr.querySelectorAll('.cv-col-cell');
...
function nvInjectArticleJsonLd(post, isoDate, description) {
    ...
    const pageUrl = index
        ? `${NV_SITE_ORIGIN}/cooking-voca.html?index=${index}`
        : `${NV_SITE_ORIGIN}/cooking-voca.html`;
    const itemList = nvParseRankingItemList(post.message);
    const payload = {
        '@context': 'https://schema.org',
        '@type': 'Article',
        headline: post.title || 'FIFA Cooking Voca',
        description,
...
function updatePageSeo(post, formattedDate) {
    ...
    const title = (post.title || 'Cooking Voca').trim();
    const desc = nvPlainText(post.message, 155);
    const fullTitle = `${title} | Cooking Voca · English Easy Study`;
...
function showError(message, details = '') {
    ...
    postContainer.innerHTML = `
        <div style="text-align: center; padding: 30px; color: #d32f2f; background-color: #ffebee; border-radius: 8px; margin: 20px;">
            ...
            <button class="btn btn-primary mt-3" onclick="window.location.href='cooking-voca-list.html${apiParam}'">목록으로 돌아가기</button>
        </div>
    `;
}
...
async function loadPost() {
    ...
    try {
        ...
        if (slug) {
            const slugUrl = `${API_BASE_URL}/cooking-voca/by-slug/${encodeURIComponent(slug)}`;
        ...
        if (!post) {
        const url = `${API_BASE_URL}/cooking-voca`;
        ...
        // 조회수 증가 API 호출
        try {
            const viewResponse = await fetch(`${API_BASE_URL}/cooking-voca/${post._id}/view`, {
                method: 'POST',
                headers: {
        ...
        if (backBtn) backBtn.onclick = () => {
            const apiParam = apiMode ? `?api=${apiMode}` : '';
            window.location.href = `cooking-voca-list.html${apiParam}`;
        };
        ...
        if (editBtn) editBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/cooking-voca-viewpost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (editForm) editForm.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/cooking-voca-updatepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (deleteBtn) deleteBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/cooking-voca-deletepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
