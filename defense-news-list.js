...
console.log('📡 국방뉴스 API_BASE_URL:', API_BASE_URL);
console.log('defense-news-list.js v20260818');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (guestbookForm) guestbookForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/defense-news`, {
                method: 'POST',
                headers: {
    ...
    async function loadMessages(forceRefresh = false) {
        ...
        try {
            ...
            let errorMessage = `요청 URL: <code>${API_BASE_URL}/defense-news</code><br><br>`;
    ...
    window.adminDeletePost = async function (id) {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/defense-news-admin/deletepost`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
