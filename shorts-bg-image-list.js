...
console.log('📡 쇼츠 배경 이미지 API_BASE_URL:', API_BASE_URL);
console.log('shorts-bg-image-list.js v20260831');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (guestbookForm) guestbookForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/shorts-bg-image`, {
                method: 'POST',
                headers: {
    ...
    async function loadMessages(forceRefresh = false) {
        ...
        try {
            ...
            let errorMessage = `요청 URL: <code>${API_BASE_URL}/shorts-bg-image</code><br><br>`;
    ...
    function renderMessages(messages, container) {
        renderGuestbookTable(container, messages, {
            postPage: 'shorts-bg-image.html',
            postPath: 'shorts-bg-image',
            apiMode: apiMode,
    ...
    window.adminDeletePost = async function (id) {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/shorts-bg-image-admin/deletepost`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
