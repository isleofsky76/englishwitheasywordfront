...
// Ranking News 시맨틱 HTML + 기본 서식 태그 허용
...
function showError(message, details = '') {
    ...
    postContainer.innerHTML = `
        <div style="text-align: center; padding: 30px; color: #d32f2f; background-color: #ffebee; border-radius: 8px; margin: 20px;">
            ...
            <button class="btn btn-primary mt-3" onclick="window.location.href='ranking-news-list.html${apiParam}'">목록으로 돌아가기</button>
        </div>
    `;
}
...
async function loadPost() {
    ...
    try {
        ...
        // 조회수 증가 API 호출
        try {
            const viewResponse = await fetch(`${API_BASE_URL}/ranking-news/${post._id}/view`, {
                method: 'POST',
                headers: {
        ...
        if (backBtn) backBtn.onclick = () => {
            const apiParam = apiMode ? `?api=${apiMode}` : '';
            window.location.href = `ranking-news-list.html${apiParam}`;
        };
        ...
        if (editBtn) editBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/ranking-news-viewpost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (editForm) editForm.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/ranking-news-updatepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (deleteBtn) deleteBtn.onclick = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/ranking-news-deletepost`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
