...
function nvParseRankingItemList(html) {
    const div = document.createElement('div');
    div.innerHTML = html || '';
    const rankLines = div.querySelectorAll('.nv-rank-line');
    if (rankLines.length) {
        return {
            '@type': 'ItemList',
    ...
    const rows = div.querySelectorAll('.nv-rank-table tbody tr');
    if (!rows.length) return null;
    return {
        '@type': 'ItemList',
        numberOfItems: rows.length,
        itemListElement: Array.from(rows).map((tr, i) => {
            const rank = tr.querySelector('.nv-col-rank')?.textContent?.trim() || String(i + 1);
            const cells = tr.querySelectorAll('.nv-col-cell');
...
function attachVocabularyWebTTS(container) {
    ...
    const paragraphs = container.querySelectorAll(
        'p, .nv-sentence-en, .nv-term, .nv-source-title, .nv-en'
    );
    paragraphs.forEach((p) => {
        ...
        const newLines = lines.map((lineHtml) => {
            ...
            if (p.classList && p.classList.contains('nv-source-text')) return lineHtml;
...
/** 표시 순서: 기사 배경 → 단어 블록 → 관련 링크(유튜브·기사) */
function enforceNewsVocaSectionOrder(article) {
    const related = article.querySelector(':scope > .nv-related-links, .nv-related-links');
    ...
    for (const el of [youtube, source]) {
        if (el) article.appendChild(el);
    }
}
...
function fixLegacyNewsVocaLayout(html) {
    ...
    root.querySelectorAll('.nv-text').forEach((article) => {
        article.querySelectorAll('.nv-word-section .nv-word, .nv-block .nv-word').forEach((word) => {
            ...
            const main = word.querySelector('.nv-word-main');

            if (main) {
                const koInMain = main.querySelector(':scope > .nv-ko');
...
async function loadPost() {
    ...
    try {
        ...
        // 이미지/동영상 링크 변환하여 표시
        const convertedMessage = fixLegacyNewsVocaLayout(convertMediaLinks(post.message || ''));
